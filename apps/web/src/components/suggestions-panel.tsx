  ...
  return (
    <section className="panel">
      ...
      <p className="muted">
        ...
        échouée). Chaque signal porte un niveau — silencieux, info, utile,
        important — qui décide de son canal : journal, notification ou
        annonce à voix haute. Sous votre seuil, rien n&apos;est perdu : c&apos;est
        journalisé ici, sans vous interrompre.
      </p>
      ...
      {active.length > 0 && (
        <table>
          <tbody>
            {active.map((s) => (
              <tr key={s.id} data-level={s.level}>
                <td>{s.message}</td>
                <td className="muted">{s.level}</td>
                <td className="muted">
                  {s.deliveredAt ? `livrée à ${s.deliveredTo}` : s.decision}
                </td>
                <td>
