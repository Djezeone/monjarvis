...
test("proactivité off : le sweep ne fait rien du tout", async ({ request }) => {
  ...
  expect(sweep).toEqual({ generated: 0, delivered: 0, capped: 0, proactivity: "off" });
});
...
test("le plafond low limite à une livraison par heure", async ({ request }) => {
  await request.put("/api/jarvis/preferences", { data: { proactivity: "low" } });
  await failACommand(request, "premier échec plafonné");
  await failACommand(request, "second échec plafonné");

  const sweep = await (await request.post("/api/jarvis/suggestions/sweep")).json();
  expect(sweep.generated).toBe(2);
  // A delivery already happened this hour (previous test): budget is 0.
  expect(sweep.delivered).toBe(0);
  expect(sweep.capped).toBe(2);
});
